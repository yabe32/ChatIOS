...
async def process_chat_payload(request, form_data, user, metadata, model):
    ...
    if user and user.role != 'admin':
        # Private deployments should treat backend configuration as authoritative.
        # Normal users may chat, but cannot override model/system/advanced params.
        default_models = [m for m in (request.app.state.config.DEFAULT_MODELS or '').split(',') if m]
        if default_models:
            form_data['model'] = default_models[0]
        ...
        form_data['messages'] = [m for m in messages if m.get('role') not in ('system', 'developer')]
